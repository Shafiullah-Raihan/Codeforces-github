...
 
	for(ll i=0;i<n;i++)
	{
		...
		if(pr.empty() || pr.back().F != arr[i])
		{
			pr.emplace_back(arr[i],1);
		}
		else{
			pr.back().S++;
		}
...
 
	...
	for(ll i=0;i<ans;i++)
	{
		if(i<ans-1 && pr[i].S>1 && (i+2>=ans || pr[i+2].F != pr[i].F))
		{
			cout<< ans+1<<endl;return;
		}
		if(i>0 && pr[i].S>1 && (i-2<0 || pr[i-2].F != pr[i].F))
		{
			cout<< ans+1<<endl;return;
		}
	}
