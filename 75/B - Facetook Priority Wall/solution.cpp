...
void raihan()
{
    ...
    for(int i=0; i<n; i++)
    {
        ...
        string ptt, person;
        if (b == "commented" || b == "posted") cin>>ptt>>person>>ptt;
        else cin>>person>>ptt;
        
        int cur = (b == "commented" ? 10 : b == "posted" ? 15 : 5);
        
        person.pop_back(), person.pop_back();
        f[person], f[a];
        if (a == s) f[person] += cur;
        else if (person == s) f[a] += cur;
    }
    
    vector<pair<int, string>> v;
    for(auto [i, j] : f) v.push_back({j, i});
    
    sort(v.begin(), v.end(), cmp);
    for(auto i : v) if (i.second != s) cout<<i.second<<endl;
